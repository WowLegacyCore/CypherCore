...
namespace Game.Maps
{
    public class Map : IDisposable
    {
        ...
        public bool IsGridLoaded(uint gridId) => IsGridLoaded(new GridCoord(gridId % MapConst.MaxGrids, gridId / MapConst.MaxGrids));

        public bool IsGridLoaded(float x, float y) => IsGridLoaded(GridDefines.ComputeGridCoord(x, y));

        public bool IsGridLoaded(Position pos) => IsGridLoaded(pos.GetPositionX(), pos.GetPositionY());

        public bool IsGridLoaded(GridCoord p)
        => (GetGrid(p.X_coord, p.Y_coord) != null && IsGridObjectDataLoaded(p.X_coord, p.Y_coord));
        ...
        public float GetWaterOrGroundLevel(PhaseShift phaseShift, float x, float y, float z, ref float ground, bool swim = false, float collisionHeight = MapConst.DefaultCollesionHeight)
        {
            if (GetGridMap(PhasingHandler.GetTerrainMapId(phaseShift, this, x, y), x, y) != null)
            {
                ...
                ZLiquidStatus res = GetLiquidStatus(phaseShift, x, y, ground_z, LiquidHeaderTypeFlags.AllLiquids, out LiquidData liquid_status, collisionHeight);
                switch (res)
                {
        ...
        => Math.Max(GetStaticHeight(phaseShift, x, y, z, vmap, maxSearchDist), GetGameObjectFloor(phaseShift, x, y, z, maxSearchDist));
        ...
        => GetHeight(phaseShift, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), vmap, maxSearchDist);
        ...
        public bool IsOutdoors(PhaseShift phaseShift, float x, float y, float z)
        {
            ...
            if (!GetAreaInfo(phaseShift, x, y, z, out uint mogpFlags, out int adtId, out int rootId, out int groupId))
                return true;
        ...
        private bool GetAreaInfo(PhaseShift phaseShift, float x, float y, float z, out uint flags, out int adtId, out int rootId, out int groupId)
        {
            ...
            bool hasDynamicAreaInfo = _dynamicTree.GetAreaInfo(x, y, ref dynamic_z, phaseShift, out uint dflags, out int dadtId, out int drootId, out int dgroupId);
        ...
        => GetAreaId(phaseShift, x, y, z, out _);
        ...
        public uint GetAreaId(PhaseShift phaseShift, Position pos) => GetAreaId(phaseShift, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
        ...
        public uint GetZoneId(PhaseShift phaseShift, Position pos) => GetZoneId(phaseShift, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
        ...
        => GetLiquidStatus(phaseShift, x, y, z, reqLiquidType, out _, collisionHeight);
        ...
        => Convert.ToBoolean(GetLiquidStatus(phaseShift, x, y, pZ, LiquidHeaderTypeFlags.AllLiquids) & (ZLiquidStatus.InWater | ZLiquidStatus.UnderWater));

        public bool IsUnderWater(PhaseShift phaseShift, float x, float y, float z)
        => Convert.ToBoolean(GetLiquidStatus(phaseShift, x, y, z, LiquidHeaderTypeFlags.Water | LiquidHeaderTypeFlags.Ocean) & ZLiquidStatus.UnderWater);

        public string GetMapName()
        => i_mapRecord != null ? i_mapRecord.MapName[Global.WorldMgr.GetDefaultDbcLocale()] : "UNNAMEDMAP";
        ...
        => SpawnGroupDespawn(groupId, deleteRespawnTimes, out _);
        ...
        public void AddToActive(WorldObject obj)
        {
            ...
            if (obj.IsTypeId(TypeId.Unit))
            {
                ...
                {
                    c.GetRespawnPosition(out float x, out float y, out _);
        ...
        public long GetRespawnTime(SpawnObjectType type, ulong spawnId) => (type == SpawnObjectType.GameObject) ? GetGORespawnTime(spawnId) : GetCreatureRespawnTime(spawnId);
        ...
        public Corpse ConvertCorpseToBones(ObjectGuid ownerGuid, bool insignia = false)
        {
                ...
                !IsRemovalGrid(corpse.GetPositionX(), corpse.GetPositionY()))
            {
                ...
                bones.SetCorpseDynamicFlags((CorpseDynFlags)(byte)corpse.GetUpdateField<uint>(CorpseFields.DynamicFlags));
                bones.SetOwnerGUID(corpse.GetUpdateField<ObjectGuid>(CorpseFields.Owner));
                bones.SetPartyGUID(corpse.GetUpdateField<ObjectGuid>(CorpseFields.PartyGUID));
                bones.SetGuildGUID(corpse.GetUpdateField<ObjectGuid>(CorpseFields.GuildGUID));
                bones.SetDisplayId(corpse.GetUpdateField<uint>(CorpseFields.DisplayID));
                bones.SetRace((Race)corpse.GetUpdateField<byte>(CorpseFields.Bytes1, 0));
                bones.SetSex((Gender)corpse.GetUpdateField<byte>(CorpseFields.Bytes1, 1));
                bones.SetClass((Class)corpse.GetUpdateField<byte>(CorpseFields.Bytes1, 2));
                bones.SetCustomizations(corpse.GetCustomizationChoices());
                bones.SetFlags((CorpseFlags)(corpse.GetUpdateField<uint>(CorpseFields.Flags) | (uint)CorpseFlags.Bones));
                bones.SetFactionTemplate(corpse.GetUpdateField<int>(CorpseFields.FactionTemplate));
                for (int i = 0; i < EquipmentSlot.End; ++i)
                    bones.SetItem(i, corpse.GetUpdateField<uint>(CorpseFields.Items + i));
        ...
        public MapRecord GetEntry() => i_mapRecord;
        ...
        public float GetVisibilityRange() => m_VisibleDistance;
        ...
        public bool IsRemovalGrid(Position pos) => IsRemovalGrid(pos.GetPositionX(), pos.GetPositionY());

        private bool GetUnloadLock(GridCoord p) => GetGrid(p.X_coord, p.Y_coord).GetUnloadLock();

        void SetUnloadLock(GridCoord p, bool on) => GetGrid(p.X_coord, p.Y_coord).SetUnloadExplicitLock(on);

        public void ResetGridExpiry(Grid grid, float factor = 1) => grid.ResetTimeTracker((long)(i_gridExpiry * factor));

        public long GetGridExpiry() => i_gridExpiry;
        ...
        public uint GetInstanceId() => i_InstanceId;

        public virtual EnterState CannotEnter(Player player) => EnterState.CanEnter;

        public Difficulty GetDifficultyID() => i_spawnMode;

        public MapDifficultyRecord GetMapDifficulty() => Global.DB2Mgr.GetMapDifficultyData(GetId(), GetDifficultyID());
        ...
        public bool IsNonRaidDungeon() => i_mapRecord != null && i_mapRecord.IsNonRaidDungeon();

        public bool IsRaid() => i_mapRecord != null && i_mapRecord.IsRaid();

        public bool IsRaidOrHeroicDungeon() => IsRaid() || IsHeroic();
        ...
        public bool Is25ManRaid() => IsRaid() && (i_spawnMode == Difficulty.Raid25N || i_spawnMode == Difficulty.Raid25HC);

        public bool IsBattleground() => i_mapRecord != null && i_mapRecord.IsBattleground();

        public bool IsBattleArena() => i_mapRecord != null && i_mapRecord.IsBattleArena();
        ...
        private bool IsCellMarked(uint pCellId) => marked_cells.Get((int)pCellId);

        void MarkCell(uint pCellId) => marked_cells.Set((int)pCellId, true);

        public bool HavePlayers() => !m_activePlayers.Empty();

        public void AddWorldObject(WorldObject obj) => i_worldObjects.Add(obj);

        public void RemoveWorldObject(WorldObject obj) => i_worldObjects.Remove(obj);

        public List<Player> GetPlayers() => m_activePlayers;

        public Dictionary<ObjectGuid, WorldObject> GetObjectsStore() => _objectsStore;

        public MultiMap<ulong, Creature> GetCreatureBySpawnIdStore() => _creatureBySpawnIdStore;

        public MultiMap<ulong, GameObject> GetGameObjectBySpawnIdStore() => _gameobjectBySpawnIdStore;

        public List<Corpse> GetCorpsesInCell(uint cellId) => _corpsesByCell.LookupByKey(cellId);

        public Corpse GetCorpseByPlayer(ObjectGuid ownerGuid) => _corpsesByPlayer.LookupByKey(ownerGuid);

        public MapInstanced ToMapInstanced() => Instanceable() ? (this as MapInstanced) : null;
        ...
        public void RemoveGameObjectModel(GameObjectModel model) => _dynamicTree.Remove(model);

        public void InsertGameObjectModel(GameObjectModel model) => _dynamicTree.Insert(model);

        public bool ContainsGameObjectModel(GameObjectModel model) => _dynamicTree.Contains(model);

        public float GetGameObjectFloor(PhaseShift phaseShift, float x, float y, float z, float maxSearchDist = MapConst.DefaultHeightSearch) => _dynamicTree.GetHeight(x, y, z, maxSearchDist, phaseShift);

        public virtual uint GetOwnerGuildId(Team team = Team.Other) => 0;

        public long GetCreatureRespawnTime(ulong dbGuid) => _creatureRespawnTimesBySpawnId.ContainsKey(dbGuid) ? _creatureRespawnTimesBySpawnId[dbGuid].respawnTime : 0;

        public long GetGORespawnTime(ulong dbGuid) => _gameObjectRespawnTimesBySpawnId.ContainsKey(dbGuid) ? _gameObjectRespawnTimesBySpawnId[dbGuid].respawnTime : 0;

        void SetTimer(uint t) => i_gridExpiry = t < MapConst.MinGridDelay ? MapConst.MinGridDelay : t;
        ...
        private bool IsGridObjectDataLoaded(uint x, uint y) => GetGrid(x, y).IsGridObjectDataLoaded();
        ...
        public Conversation GetConversation(ObjectGuid guid) => (Conversation)_objectsStore.LookupByKey(guid);
        public Player GetPlayer(ObjectGuid guid) => Global.ObjAccessor.GetPlayer(this, guid);
        ...
        => type == SpawnObjectType.GameObject ? GetGameObjectBySpawnId(spawnId) : GetCreatureBySpawnId(spawnId);
        ...
        => GetGuidSequenceGenerator(high).Generate();
        ...
        public void AddUpdateObject(WorldObject obj) => _updateObjects.Add(obj);

        public void RemoveUpdateObject(WorldObject obj) => _updateObjects.Remove(obj);

        public static implicit operator bool(Map map) => map != null;
    ...
    public class InstanceMap : Map
    {
        ...
        public string GetScriptName() => Global.ObjectMgr.GetScriptName(i_script_id);
        ...
        public uint GetScriptId() => i_script_id;

        public InstanceScript GetInstanceScript() => i_data;
    ...
    public class BattlegroundMap : Map
    {
        ...
        public void SetUnload() => m_unloadTimer = 1;
        ...
        public Battleground GetBG() => m_bg;
