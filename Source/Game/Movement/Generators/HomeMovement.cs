...
namespace Game.AI
{
    public class HomeMovementGenerator<T> : MovementGeneratorMedium<T> where T : Creature
    {
        ...
        void SetTargetLocation(T owner)
        {
            ...
            if (owner.GetMotionMaster().Empty() || !owner.GetMotionMaster().Top().GetResetPosition(owner, out float x, out float y, out float z))
            {
                owner.GetHomePosition(out x, out y, out z, out float o);
                init.SetFacing(o);
            }
            init.MoveTo(x, y, z);
            init.SetWalk(false);
